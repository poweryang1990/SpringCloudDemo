    ...
    public class SwaggerParser
    {
        ...
        internal string GetTypeName(JToken token, out bool isNullable)
        {
            ...
            if (type.Value.Equals("array"))
            {
                ...
                return $"List<{this.GetTypeName(jToken, out throwawayNullable)}>";
            }
        ...
        public string FixTypeName(string input)
        {
        ...
        public string GetDefaultType(Parameter x)
        {
            ...
            var output = x.CollectionFormat == "multi" ? $"List<{typeName}>" : typeName;
            return output;
        }

        public string GetDefaultValue(Parameter x)
        {
            if (!x.Type.IsNullableType && x.CollectionFormat != "multi" && x.Type.EnumValues != null && x.Type.EnumValues.Any())
            {
                return $"{x.Type.TypeName}.{x.Type.EnumValues.FirstOrDefault()}";
            }

            return "null";
        }

        public string GetParameterType(Parameter x)
        {
        ...
        bool IsIntrinsicType(string typeName)
        {
